...
on the quotient of the product of true-model likelihoods, over the
product of alternative-model likelihoods (both using the same
...
log-likelihood ratios of the single samples, under consideration.
When this whole sum is greater than zero (i.e.\ $\log(1)$), we choose
the true model; otherwise, we choose we choose the alternative model.
...
true model to the alternative model (defined as the expected value of
this same quantity) \cite{burnham2002}.  While such a Monte Carlo
estimate can be either positive, negative, or zero, the actual value of
the Kullback-Leiber divergence always remains nonnegative---and
...
different predictions (e.g.\ for two baseball models, at least one
transition probability must be different).  In other words, our test
is {\em consistent} \cite{burnham2002}.
