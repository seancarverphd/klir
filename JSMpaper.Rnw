...
table of sums of log-likelihood ratios (Table 1).  The first column
contains single log-likelihood ratios; the second contains sums of
two; the third contains sums of three; and so on.  The entries in the
table derive from independent random samples, and independence holds
across rows, as well.  For large enough matrices, the computational
...
fill in the table.  This procedure introduces a bias, but one
...
\begin{table}
  ...
  \caption{The {\em bootstrap table}: each entry is the sum of
    ...
    large population (in this case, size 200,000) drawn from the true model, t(5).  
    ...
    The $k^{th}$ column contains the sum of $k$ of these log-likelihood
...
<<proportioncorrect, echo=F, fig.cap='The proportion of correct selections from a ``bootstrap table" with 1000 rows and 250 columns), drawn from a bootstrap population of size 200,000 (see Table 1). The column number of the table equals the sample size used to make the selection (horizontal axis).  The proportion correct (vertical axis) is the ratio of the number positive entries in each column (each indicating a correct selection) to the total number of rows.  Thus each proportion correct is necessarily a multiple of the reciprocal of the number of rows in the table.  As the proportion correct crosses the value of 0.95, (boundary between yellow and green)the confidence level is deemed acceptable.'>>=
...
@
% 
<<fifthpercentile, echo=F, fig.cap='The fifth percentile (vertical axis) of the log-likelihood ratio within each column of a bootstrap table with 1000 rows and 250 columns, drawn from bootstrap population of size 200,000.  The column number of the table equals the sample size used to make the selection (horizontal axis).  The fifth percentile crosses zero (boundary between yellow and green) at the same place where the proportion correct crosses 0.95 (Figure \ref{fig:proportioncorrect}).'>>=
