...
The mean of these log-likelihood ratios is an unbiased estimate of the
so-called {\em Kullback-Leibler divergence} (defined as the expected
...
at least one transition probability must be different).  In other
words, our test is {\em consistent} \cite{burnham2002}.
