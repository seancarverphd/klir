...
density function (Figure ??).  This graph for N(0,1) is just the
familiar {\em bell curve}; the graphs for the other t-distributions
appear nearly indistinguishable from the one for N(0,1), but they are
slightly warped bells, as described below.
