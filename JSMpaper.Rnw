...
<<wordcloudtransitions, echo=F, fig.cap='The baseball state transitions that occurred throughout the 2011 Major League season, arranged randomly, and sized according to their frequency of occurrence.'>>=
...
<<wordcloudhalfinnings, echo=F, fig.cap='Most common half-innings, as predicted from transition probabilities fit with 2011 Major League season data, arranged randomly, and sized according to predicted frequency of occurrence.  Half-innings with predicted probabilities less than 0.01 are lumped together as ``Rare Half Innings."'>>=
set.seed(3)
require(wordcloud)
...
as well.  For large enough matrices, the computational requirements
...
\begin{table}
  ...
  \caption{The bootstrap table: each entry in the table is the sum of
    log-likelihood ratios computed from samples bootstrapped from a
    large population drawn from the true model.  A single log-likelihood
    ratio is the log of the ratio of the probability density of the
    true model at the sample, to the probability density of the
    alternative model at the sample.  The sum of a number of
    log-likelihood ratios is the log-likelihood ratio of that ensemble.}
...
<<proportioncorrect, echo=F, fig.cap='The proportion correct in each column of a bootstrap table with 1000 rows and 250 columns.  The bootstrap population size is 200,000. The column number is the sample size used to make the selection (horizontal axis).  The proportion correct (vertical axis) is the ratio of the number positive entries in each column (indicating the number of correct selections) to the total number of rows.  Thus each proportion correct is necessarily a multiple of the reciprocal of the number of rows in the table.'>>=
...
@
% 
<<fifthpercentile, echo=F, fig.cap='The fifth percentile (vertical axis) of the log-likelihood ratio within each column of a bootstrap matrix with 1000 rows and 250 column.  The bootstrap population size is 200,000.  The column number is the sample size (horizontal axis) of the ensemble used to compute each log-likelihood ratio in the column.'>>=
...
@ 
<<roiplot, echo=F, fig.cap='The region of interest (zoom in) of the fifth percentile plot (from Figure \ref{fig:fifthpercentile}).  I define the region of interest to include the last sample before the first zero-crossing, and at the first sample after the last crossing, as well as everything in between, but nothing else.  The blue line is the linear least-squares regression line for the data in this region, the green arrow points to the estimate of the number of samples needed for 95\% confidence.'>>=
first <- roi$samples[1]
...
ggplot(data=data.frame(Sample.Size=roi$samples,Fifth.Percentile=roi$quantiles, Best.Fit=eq$est.quantiles), aes(x=Sample.Size)) +
    ...
    annotate("segment", x=needed, xend=needed, y=-0.4, yend=-0.025, color="darkgreen", alpha=I(.6), arrow=arrow()) +
    annotate("text", x=needed+5, y=-0.4, color="darkgreen", alpha=I(.6), label="Number of samples needed\nfor 95% confidence")
@
<<estimatorsviolin, echo=F, fig.cap='The caption.'>>=
...
On one hand, the lm command occasionally fails, producing wrong or
even nonsensical (negative, not shown) results.  However,
unfortunately, the quantreg:rq() method takes longer: the computation
also requires a region of interest which in turn requires duplicating
an intermediate step (calculating the quantiles).
...
distribution (mean $\pm$ standard error, $\Sexpr{length(slr.reps)}$
samples, shown in Figure \ref{fig:estimatorsviolin}).  There was only
a slight difference with the lm() function:
$\Sexpr{mean.slr} \pm \Sexpr{se.slr}$, (same sample size, and also
shown in Figure \ref{fig:estimatorsviolin}).  After eliminating the
two extreme outliers from the distribution, the result changes to
$\Sexpr{mean.slr.na} \pm \Sexpr{se.slr.na}$.

Each set of computations for a $\pm 1$ range of estimates took minutes
(not hours) on a MacBook Pro.  On the other hand, I calculate that it
takes $\Sexpr{mean.bb} \pm \Sexpr{se.bb}$ half-innings, simulated from
the MLE model derived from the 2011 Baltimore Orioles, batting at
home, to reject, with 95\% confidence, the statment that these
half-innings were sampled from the 2011 New York Yankees MLE model.
This calculation was somewhat faster and required only 6 samples.
...
https://github.com/maxtoki/baseball\_R.  We provide our code, and the
source for this paper, available at:
https://github.com/seancarverphd/klir.  We use best practices for
reproducible research.  Note that the first GitHub repository also
contains the 2011 baseball data needed to run our code.  \nocite{*}
