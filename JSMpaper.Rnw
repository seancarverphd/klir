...
\begin{table}
  ...
  \caption{The {\em bootstrap table}: each entry is the sum of
    ...
    large population drawn from the true model, t(5).  A single log-likelihood
    ...
    alternative model, N(0,1), at the sample.  The sum of a number of these values, for an ensemble of independent samples, is 
    the log-likelihood ratio of that ensemble.
    The $k^{th}$ column contains the sum of $k$ log-likelihood
    ratios.  A positive entry indicates a correct selection with the
