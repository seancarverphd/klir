...
can make the choice confidently.  Indeed, you can have as high a
chance as you want, short of being absolutely certain, of choosing the
right team, provided you have enough data [Burnham \& Andersen].  But
how much data suffice for the confidence you demand?
...
(sums of certain entries equal 1) stem from the stipulation that, from
each of the 24 transient states (i.e.\ other than XXX), the game must,
with certainty, proceed.
...
play.  The park effects add distinctiveness to the models that also
...
the teams under study.  For this reason, the interpretation of our results,
below, will be made in the context of this rather unfortunate behavior
of the MLEs for baseball.

\section{Deciding Between Models}
Baseball presents one example; below we introduce another.  Our
calculations work for with any type of model, provided (1) you can
simulate the true model to generate samples, and (2) you can calculate
the likelihood of each sample, both for the true model, and for any
alternative model under consideration.  The relative values of these
likelihoods determine which model you deem as correct.  Of course,
...
records of a half-inning of the plays for the team at bat.  A
simulation starts with bases empty and no outs (the 0 state) then
successively applies the transition probabilities to ``throw the
dice,'' to see the succession of states, until reaching the end of the
inning (the XXX state).
...
(for all 2011 Major League teams) predict 0:0X:0XX:XXX for 31.5\% of
the half-innings.  Individual teams will have a different percentages
for this occurrence, higher or lower, depending on the skill of their
batters.  True, this most common outcome does not represent a
majority of all half-innings, but the next most common half-inning,
...
than 1\% of the time).  The rest I call {\em rare innings}.  But if we
sum the probabilities for the non-rare innings we only get 0.555,
which means that 44.5\% of the time, we predict a rare inning.  As a
group, rare innings are not particularly rare, but each one occurs
less than once in 100 half-innings.  How is this result consistent
with reality?  Realize that there are infinitely many possible
half-innings, provided the team at bat manages to avoid three outs,
and the referee does not call the game as the score gets increasingly
lopsided.  The probabilities of all of these possible innings sum
exactly to 1 in an infinite convergent series.  The fact that
individual rare inning have such low probabilities is made up for by
the fact that there are so many of these rare innings.
...
of the t-distribution is a single number on the real line, either
positive, negative, or zero (such as -0.143134).  Consequently, unlike
in baseball with its discrete half-innings, for the t-distributions,
the samples occur on a continuum---in one dimension.  As with
baseball, the next step grasps the likelihoods of the different
possibilities.  For the t-distributions, the simplicity of the sample
space allows you to make graph (known as the probability density
function).  Typically, the sample goes on the horizontal axis, and the
likelihood (or probability density) goes on the vertical axis.  For
N(0,1), the graph is just the familiar {\em bell curve}; for the other
t-distributions, the graph appears as a nearly indistinguishable, but
slightly warped, bell, as described below.
...
the probabilities of the different innings summed to 1, the
probability density of the t-distribution integrates to 1 (i.e.\ area
under the bell equals 1) over the whole range of possible samples.
Because the range of possible t-samples has no limit, the tails must
drop off precipitously to keep the area below them finite, just as the
probabilities for half-innings with higher and higher scores must
similarly drop off.  The key point is that this drop off occurs slower
for t(5) than for N(0,1): t-distributions have heavier tails with
smaller degrees of freedom.  In a similar fashion, in baseball the
``tail'' (by which I mean: the probabilities of half-innings with
higher and higher scores) drops off slower for better teams.  But
higher probability in the ``tails'' must be compensated for by lower
probability in the ``center.''  The top of the bell is higher for
N(0,1) than for the heavier tailed t(5).  Likewise, the lowest scoring
half-innings, such as 0:0X:0XX:XXX, are more likely for worse teams
than for better teams.
...
t-distributions, if the sample is in the tails, you pick t(5); if the
sample is in the center, you pick N(0,1).  The intersections of the
pair's probability density functions determine the boundaries of the
regions where the different models are selected.  For baseball, you
will tend to pick a better team if the half-inning's score is higher,
and a worse team if the score is lower, although the judgement based
on likelihood is often more subtle than the one just described.

In many cases, based on one sample, your answer will be wrong, most of
the time.  Indeed for all t-distributions, including t(5), most
samples are near the center (top of the bell), so t(5) will be
rejected in favor of N(0,1) with one sample, most of the time, even if
it is the true model.  Likewise good teams do often get low scoring
half-innings, which can deem them less likely than worse teams.  It
takes many samples including rarities in the tails, to fill in the
histogram and make the correct choice.

How do we compute likelihood?  For t-distributions, it is the height
of the (warped) bell curve at the sample, given by formulas (for the
appropriate probability density function) easily found online.  In
this respect, baseball is simpler: the likelihood of a half inning is
the product of all its transition probabilities.  In both cases, to
compute the likelihood of an ensemble of samples, you compute the
product of the likelihoods of the individual samples.  Products appear
because of the independence (or conditional independence) of the
events under consideration.  Note that, in working with many samples,
you should, instead, work with the log-likelihoods, otherwise the
numbers will get too small for a computer's floating point arithmetic
...
is equivalent making the decision based on the much championed {\em
...
the same data used for fitting the models [Burnham \& Anderson].  We
make our selection with data simulated after we fix the parameters, so
in the formula for AIC, the number of fitted parameters is 0, which
implies that the two methods agree.

As suggested by the name {\em likelihood ratio test} we use ratios.
Specifically, we use the product of the likelihoods of the true model,
over the product of the likelihoods of the alternative model, where
each factor in each product evaluates a likelihood of one of the
samples under consideration from the true model.  If this quantity is
greater that 1, we choose the true model; otherwise we choose the
alternative model.  After taking logs, these products and quotients
simplify to the sum of the log-likelihood ratios of the single
samples, under consideration.  When this whole sum is greater than
zero (i.e.\ $\log(1)$), we choose the true model; otherwise, we
choose we choose the alternative model.

The mean of these same log-likelihood ratios is an unbiased estimate
of the so-called Kullback-Leibler divergence (defined as the expected
value of this same quantity) [Burnham \& Andersen].  While such a
Monte Carlo estimate can be either positive, negative, or zero, the
true value of the Kullback-Leiber divergence always remains
nonnegative---and positive as long as the models make different
predictions (known together as the {\em Gibbs inequality}).  From this
result, and the central limit theorem, that we can be sure that we will,
with any level of confidence short of certainty, select the true
model, with enough samples, provided the two models make different
predictions (e.g.\ for baseball models, at least one transition
...
confidence level (e.g.\ 0.95) {\em should} be my answer.  In practice,
however, this crossing is not well defined because the proportion
varies randomly, and in a way that varies across columns.  As a
result, there appears a region, which I call the {\em region of
  interest}, where the desired crossing occurs many times.

My first solution was to zoom in on the region of interest and perform
a linear regression on the proportion correct to pinpoint an estimate
...
percentile of the log-likelihood ratio crosses 0 (the threshold for
the sum of the log-likelihoods to indicate a correct selection).  As
...
regression.  But the quantiles are no longer discrete and the
regression performs better.
...
the sums of log-likelihoods matrix, with the R command quantreg::rq())
both using the default arguments, where possible.  A careful reading
of the documentation reveals fundamental differences in between these
two procedures, however, there was not a substantial difference in the
...
because of having to compute quantiles twice, rq() takes somewhat
longer.
...
in 2011.  If one of these innings occurs in a set of Baltimore at bat
samples, the selection is immediately made for the true model, the
...
[Marchi \& Albert, 2013], but they are beyond the scope of the present
paper.
...
Kullback-Leibler divergence.  However, our measure is discrete, unlike
...
code from the GitHub companion to [Marchi \& Albert] (with the data):
https://github.com/maxtoki/baseball\_R.  Our code:
https://github.com/seancarverphd/klir.
