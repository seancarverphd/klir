...
source('simulation.R')
@
<<loadshort,echo=F,include=F,cache=F>>=
source('msbaseball.R')
source('wcloud.R')
source('reps.R')
@ 
...
percentage.rare <- 100*(1-float.probability.nonrare)
@ 
...
stopifnot(all.equal(percentage.rare,44))
@ 
\section{Motivation}
...
of the time, Figure ??).  I call these {\em non-rare half-innings};
the rest I call {\em rare half-innings}.  But the probabilities for
the non-rare half-innings only sum to \Sexpr{probability.nonrare},
which predicts a rare half-inning \Sexpr{percentage.rare}\% of the
time.  As a group, rare half-innings are not particularly rare, but
each one occurs less than once in 100 half-innings.  How is this
statement consistent with reality?  Realize that, with no cap on the
score, there are infinitely many possible half-innings; their
probabilities sum to exactly 1 in an infinite convergent series.
Necessarily, these probabilities tail off to keep the sum finite, but
still, the fact that individual half-innings have such low
probabilities is made up for by the fact that there are so many of
them.
...
each column that are positive (indicating a correct selection).  The
