...
```{r numberofsamples, echo=TRUE, cache=FALSE}
samples.needed(ll.t5.t5, ll.t5.normal,
                  max.samples=250, bootstrap.rows=1000, seed=5,
                  confidence.level=0.95, quantile.regression=FALSE)
```
- The following code repeats the same experiment shown above, each time drawing a new sample for bootstrapping and computing new likelioods.
```{r repssetup}
source('reps.R')
```
```{r repeated, echo=TRUE, cache=FALSE}
