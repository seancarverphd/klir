...
list.from.string <- function(from.string) {
  batter <- TRUE
...
list.transitions <- function(batter, runner1, runner2, runner3, outs) {
  ...
  if (isTRUE(batter)) {
    ...
    for (new.batter in 0:4) {
      trans.list <- c(trans.list, list.transitions(new.batter, runner1, runner2, runner3, outs))
    }
  }
  if (isTRUE(runner1)) {
    do.return <- TRUE
    for (new.runner1 in 0:4) {
      print(c("batter",batter,"new.runner1",new.runner1))
      trans.list <- c(trans.list, list.transitions(batter, new.runner1, runner2, runner3, outs))
    }
  }
  if (isTRUE(runner2)) {
    do.return <- TRUE
  ...
  legit <- ((batter < runner1) | num.equal(runner1, 0) | identical(runner1, FALSE)) &
           ((batter < runner2) | num.equal(runner2, 0) | identical(runner2, FALSE)) &
           ((batter < runner3) | num.equal(runner3, 0) | identical(runner3, FALSE)) &
           ((runner1 < runner2) | num.equal(runner2, 0) | identical(runner1, FALSE) | identical(runner2, FALSE)) &
           ((runner1 < runner3) | num.equal(runner3, 0) | identical(runner1, FALSE) | identical(runner3, FALSE)) &
           ((runner2 < runner3) | num.equal(runner3, 0) | identical(runner2, FALSE) | identical(runner3, FALSE))
  if (legit) {
